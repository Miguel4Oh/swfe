...
function llenarCampos(conceptos) {
    ...
    if (conceptos.length === 0) {
        ...
        llenarTabla(conceptos);
    }
}

function llenarTabla(conceptos) {
    const tbody = document.querySelector('.cuerpo-tabla');

    tbody.innerHTML = '';

    conceptos.forEach(concepto => {
        console.log(concepto.clienteId);
        const tr = document.createElement('tr');

        tr.innerHTML = `
                <td>${concepto.nombreDepreciacion}</td>
                <td>${concepto.porcentajeDepreciacion}</td>
                <td>
                     <a class="enlace-contents"
                        href="/swfe/conceptos/concepto-form?concepto-id=${concepto.conceptoDepreciacionId}">
                     ...
                     <a class="enlace-contents" href="/swfe/conceptos/eliminar?concepto-id=${concepto.conceptoDepreciacionId}">
